...
import { getBrandDescriptor, localeNames, type Locale } from "@/lib/i18n";
...
export function Header({ locale, copy }: { locale: Locale; copy: Copy }) {
  return <header className="border-b border-slate-200 bg-white">
    <div className="shell flex min-h-16 items-center justify-between gap-4">
      <Link href={`/${locale}/`} className="focus-ring flex min-w-0 items-center gap-2"><img src="/brand/navgrove-logo.png" alt="NavGrove" className="h-10 w-auto max-w-40 object-contain sm:max-w-48" /><span className="hidden whitespace-nowrap text-sm font-medium text-slate-500 sm:inline">{getBrandDescriptor(locale)}</span></Link>
